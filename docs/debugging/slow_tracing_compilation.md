...
* **`jax_log_compiles`**: Logs elapsed time for tracing, lowering, and
  ...
  are still logged a debug messages.
...
### Eager Op-by-Op Compilations (The "Dozens of Tiny Modules" Symptom)
...
```
* **Why it fails:** JAX indexes its in-memory tracing cache using th
...
### Gotcha: Eager Python Loops (The Initialization Trap)

Iterating over large Python container of weights to apply normalization
or quantization in eager mode forces XLA to compile a separate binary for
every unique weight matrix shape in the model.
...
JAX dumps a pprof-compatible profile where each sample corresponds to a
primitive equation in the Jaxpr.
...
*(In the pprof web UI flame graph, this appears as a single massive
 horizontal bar taking up 99.98% of the total profile width,
 instantly pinpointing line 8).*
...
* **The Fix:** Rewrite your code to make use of
  ...
  or avoid wrapping the loop with `@jax.jit`
  (you can still use JIT-decorated functions *inside* the loop).
